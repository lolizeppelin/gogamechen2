...
class Gogamechen2DBClient(GopDBClient, GopCdnClient):
    ...
    objfile_path_ex = '/gogamechen2/objfiles/%s/%s'

    groups_path = '/gogamechen2/groups'
    group_path = '/gogamechen2/groups/%s'
    group_path_ex = '/gogamechen2/groups/%s/%s'

    agents_chioces_path = '/gogamechen2/%s/agents'
    databases_chioces_path = '/gogamechen2/%s/databases'

    merge_path = '/gogamechen2/merge'
    continue_merge_path = '/gogamechen2/merge/%s'
    finsh_merge_path = '/gogamechen2/finish/%s'
    mergeing_path = '/gogamechen2/mergeing/%s/%s'

    appentitys_path = '/gogamechen2/group/%s/%s/entitys'
    appentity_path = '/gogamechen2/group/%s/%s/entitys/%s'
    appentity_path_ex = '/gogamechen2/group/%s/%s/entitys/%s/%s'

    bond_path = '/gogamechen2/entity/%s'
    appentitys_all_path = '/gogamechen2/entitys'

    all_packages_path = '/gogamechen2/packages'
    group_resources_path = '/gogamechen2/group/%s/resources'
    group_resource_path = '/gogamechen2/group/%s/resources/%s'
    packages_path = '/gogamechen2/group/%s/packages'
    package_path = '/gogamechen2/group/%s/packages/%s'
    package_path_ex = '/gogamechen2/group/%s/packages/%s/%s'

    packagefiles_path = '/gogamechen2/package/%s/pfiles'
    packagefile_path = '/gogamechen2/package/%s/pfiles/%s'

    def objfiles_index(self, body=None):
        resp, results = self.get(action=self.objfiles_path, body=body)
        if results['resultcode'] != common.RESULT_SUCCESS:
            raise ServerExecuteRequestError(message='list gogamechen2 objfiles fail:%d' % results['resultcode'],
    ...
    def objfile_create(self, objtype, subtype, version, fileinfo, body=None):
        ...
        resp, results = self.retryable_post(action=self.objfiles_path,
                                            body=body)
        if results['resultcode'] != common.RESULT_SUCCESS:
            raise ServerExecuteRequestError(message='create gogamechen2 objfiles fail:%d' % results['resultcode'],
                                            code=resp.status_code,
                                            resone=results['result'])
        return results
    ...
    def objfile_show(self, md5):
        ...
        if results['resultcode'] != common.RESULT_SUCCESS:
            raise ServerExecuteRequestError(message='show gogamechen2 objfiles fail:%d' % results['resultcode'],
                                            code=resp.status_code,
                                            resone=results['result'])
        return results
    ...
    def objfile_delete(self, md5):
        ...
        if results['resultcode'] != common.RESULT_SUCCESS:
            raise ServerExecuteRequestError(message='delete gogamechen2 objfiles fail:%d' % results['resultcode'],
                                            code=resp.status_code,
                                            resone=results['result'])
        return results
    ...
    def objfile_send(self, md5, objtype, body=None):
        ...
        if results['resultcode'] != common.RESULT_SUCCESS:
            raise ServerExecuteRequestError(message='send gogamechen2 objfiles fail:%d' % results['resultcode'],
                                            code=resp.status_code,
                                            resone=results['result'])
        return results
    ...
    def groups_index(self, body=None):
        ...
        if results['resultcode'] != common.RESULT_SUCCESS:
            raise ServerExecuteRequestError(message='list gogamechen2 group fail:%d' % results['resultcode'],
                                            code=resp.status_code,
                                            resone=results['result'])
        return results
    ...
    def groups_create(self, name, desc=None):
        ...
        if results['resultcode'] != common.RESULT_SUCCESS:
            raise ServerExecuteRequestError(message='create gogamechen2 group fail:%d' % results['resultcode'],
                                            code=resp.status_code,
                                            resone=results['result'])
        return results
    ...
    def group_show(self, group_id, detail=False):
        ...
        if results['resultcode'] != common.RESULT_SUCCESS:
            raise ServerExecuteRequestError(message='show gogamechen2 group fail:%d' % results['resultcode'],
                                            code=resp.status_code,
                                            resone=results['result'])
        return results
    ...
    def group_delete(self, group_id, body=None):
        ...
        if results['resultcode'] != common.RESULT_SUCCESS:
            raise ServerExecuteRequestError(message='delete gogamechen2 group fail:%d' % results['resultcode'],
                                            code=resp.status_code,
                                            resone=results['result'])
        return results
    ...
    def group_area(self, group_id, body=None):
        ...
        if results['resultcode'] != common.RESULT_SUCCESS:
            raise ServerExecuteRequestError(message='change gogamechen2 group area fail:%d' % results['resultcode'],
                                            code=resp.status_code,
                                            resone=results['result'])
        return results
    ...
    def group_maps(self, group_id, body=None):
        ...
        if results['resultcode'] != common.RESULT_SUCCESS:
            raise ServerExecuteRequestError(message='get gogamechen2 group maps fail:%d' % results['resultcode'],
                                            code=resp.status_code,
                                            resone=results['result'])
        return results
    ...
    def group_chiefs(self, group_id, body=None):
        ...
        if results['resultcode'] != common.RESULT_SUCCESS:
            raise ServerExecuteRequestError(message='get gogamechen2 group chiefs fail:%d' % results['resultcode'],
                                            code=resp.status_code,
                                            resone=results['result'])
        return results
    ...
    def group_areas(self, group_id, body=None):
        ...
        if results['resultcode'] != common.RESULT_SUCCESS:
            raise ServerExecuteRequestError(message='get gogamechen2 group areas fail:%d' % results['resultcode'],
                                            code=resp.status_code,
                                            resone=results['result'])
        return results
    ...
    def group_packages(self, group_id, body=None):
        ...
        if results['resultcode'] != common.RESULT_SUCCESS:
            raise ServerExecuteRequestError(message='get gogamechen2 group packages fail:%d' % results['resultcode'],
                                            code=resp.status_code,
                                            resone=results['result'])
        return results
    ...
    def group_databases(self, group_id, body=None):
        ...
        if results['resultcode'] != common.RESULT_SUCCESS:
            raise ServerExecuteRequestError(message='get gogamechen2 group databases fail:%d' % results['resultcode'],
                                            code=resp.status_code,
                                            resone=results['result'])
        return results
    ...
    def bondto(self, entity, databases):
        resp, results = self.post(action=self.bond_path % str(entity), body=dict(databases=databases),
                                  timeout=15)
        if results['resultcode'] != common.RESULT_SUCCESS:
            raise ServerExecuteRequestError(message='bond gogamechen2 databases fail:%d' % results['resultcode'],
                                            code=resp.status_code,
                                            resone=results['result'])
        return results
    ...
    def appentitys(self, entitys):
        resp, results = self.get(action=self.appentitys_all_path, body=dict(entitys=entitys),
                                 timeout=15)
        if results['resultcode'] != common.RESULT_SUCCESS:
            raise ServerExecuteRequestError(message='get gogamechen2 entitys fail:%d' % results['resultcode'],
                                            code=resp.status_code,
                                            resone=results['result'])
        return results
