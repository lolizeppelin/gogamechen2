...
def agent_and_database():
    cmd = 'gogamechen2-select  agents --format json'
    sub = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE)
    agents = sub.communicate()[0]
    agents = json.loads(agents)
    agents = Select(agents)

    cmd = 'gogamechen2-select  databases --format json'
    sub = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE)
    _databases = []
...
def main():
    ...
    for show_id in range(start, end+1):
        ...
        cmd = '/usr/bin/gogamechen2-appentity create -o gamesvr --appfile %(appfile)s --opentime %(opentime)d ' \
